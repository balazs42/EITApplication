...
namespace BusinessLayer
{
    public class ReconstructionPersistence : IReconstructionPersistence
    {
        ...
        public ReconstructionResult RunFemIterations(int iterationCount,
                                                     ...
                                                     double redularizationStepSize)
        {
            ...
            if (!_initialized || _discretization is not FEMMesh femMesh)
                throw new InvalidOperationException("FEM reconstruction must be initialised before running iterations.");

            _gradientStepSize = gradientStepSize;
            _regularizationWeight = redularizationStepSize;

            bool previousStopRequested = _stopRequested;
            try
            {
                _stopRequested = false;
                return RunFemReconstruction(femMesh, iterationCount);
            }
            finally
            {
                _stopRequested = previousStopRequested;
            }
        }
        ...
        private (Dictionary<int, double> Gradient, List<ReconstructionFrame> Frames) ComputeFemInverseStepsOmp(
            FEMMesh mesh,
            ...
            int electrodeCount)
        {
            ...
            var producedFrames = frames
                .Where(frame => frame != null)
                .Select(frame => frame!)
                .ToList();

            return (gradientAccumulator, producedFrames);
        }
