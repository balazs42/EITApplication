...
namespace DataAccessLayer
{
    ...
    /// Responsibilities:
    /// - Save FEM meshes as ASCII STL with a JSON description sidecar
    /// - Load FEM meshes from legacy <c>.eitmesh</c>, authored STL+description packages, or generic STL files
    /// - Save/load native <c>.eitmesh</c> XML files for LBM discretizations
    ...
    public class MeshRepository : IMeshRepository
    {
        ...
        /// <summary>
        /// Saves a FEM mesh as an ASCII STL file plus a JSON sidecar description.
        /// The sidecar is the authoritative persistence format for FEM mesh round-tripping.
        /// </summary>
        ...
        public void SaveFEMMesh(FEMMesh mesh, string name)
        {
            ...
            string stlPath = SaveFemMeshAsStl(mesh, name, "fem", timestamp);
            var description = FemMeshDescriptionSerializer.Create(mesh,
                                                                  name,
        ...
        /// with deserialized metadata. Supports legacy native <c>.eitmesh</c> files and STL packages.
        /// </summary>
        ...
        /// <summary>
        /// Deletes a mesh file and any paired sidecar/companion files generated by the application.
        ...
        public void DeleteMesh(string filePath)
        {
            ...
            else if (string.Equals(Path.GetExtension(filePath), ".stl", StringComparison.OrdinalIgnoreCase))
            {
                var jsonFile = Path.ChangeExtension(filePath, ".json");
                if (!string.IsNullOrEmpty(jsonFile) && File.Exists(jsonFile))
                    File.Delete(jsonFile);
        ...
        /// <summary>
        /// Loads a FEM mesh from legacy native <c>.eitmesh</c> XML or STL.
        /// STL loading prefers the authored JSON sidecar when present and falls back to generic STL reconstruction otherwise.
        /// </summary>
        ...
        /// for display. When an authored JSON description exists it is used as the primary metadata source;
        /// otherwise ASCII/Binary STL heuristics are used.
        /// </summary>
        private static DiscretizationInfo? TryBuildStlDiscretizationInfo(string filePath)
        {
            try
            {
                var authoredDescription = FemMeshDescriptionSerializer.TryRead(filePath);
                if (authoredDescription != null)
                {
                    var authoredMetadata = new DiscretizationMetaData
                    {
                        CreatedOn = authoredDescription.Mesh.Metadata.CreatedOn,
                        Generator = authoredDescription.Mesh.Metadata.Generator,
                        ElementCount = authoredDescription.Mesh.Metadata.ElementCount,
                        Parameters = authoredDescription.Mesh.Metadata.Parameters != null
                            ? new Dictionary<string, string>(authoredDescription.Mesh.Metadata.Parameters)
                            : new Dictionary<string, string>()
                    };

                    if (!string.IsNullOrWhiteSpace(authoredDescription.StlFileName))
                        authoredMetadata.Parameters["source"] = authoredDescription.StlFileName!;
                    authoredMetadata.Parameters["descriptionSchema"] = FemMeshDescriptionSerializer.SchemaVersion;

                    string authoredDisplayName = string.IsNullOrWhiteSpace(authoredDescription.Name)
                        ? Path.GetFileNameWithoutExtension(filePath)
                        : authoredDescription.Name;
                    return new DiscretizationInfo(authoredDisplayName, filePath, authoredMetadata);
                }
                ...
                // Append an explicit suffix so raw STL snapshots remain distinguishable from authored
                // STL+description packages that already carry richer metadata.
        ...
        /// <summary>
        /// Loads a FEM mesh from STL. If an authored JSON description is present it is used to restore the
        /// exact FEM state; otherwise the loader falls back to generic STL reconstruction and optional Matlab JSON augmentation.
        /// </summary>
        private static FEMMesh LoadFemMeshFromStl(string filePath)
        {
            var authoredDescription = FemMeshDescriptionSerializer.TryRead(filePath);
            if (authoredDescription != null)
                return FemMeshDescriptionSerializer.RehydrateMesh(authoredDescription);
