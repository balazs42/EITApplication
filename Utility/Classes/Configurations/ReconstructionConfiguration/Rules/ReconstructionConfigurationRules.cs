...
namespace Utility.Classes.Configurations.ReconstructionConfiguration.Rules
{
    ...
    public static class ReconstructionConfigurationRules
    {
        ...
        public static bool IsConnectionAllowed(BlockType source, BlockType target, out string? reason)
        {
            ...
            if(source == BlockType.ErrorMetric && target != BlockType.Optimizer)
            {
                reason = "Error Metric outputs can only feed to Optimizer blocks.";
                return false;
            }
            ...
            if(source == BlockType.PostProcessing && target != BlockType.Model)
            {
                reason = "Post-processing outputs can only feed to Model block.";
                return false;
            }
            ...
            if (target == BlockType.Optimizer &&
                source != BlockType.ErrorMetric && source != BlockType.Regularizer)
            {
        ...
        public static IReadOnlyList<string> Validate(IEnumerable<ReconstructionConfigurationBlock> blocks, IEnumerable<ReconstructionConnection> connections)
        {
            ...
            foreach(var solver in blocks.Where(b => b.Type == BlockType.Solver))
                if(!connections.Any(c => c.Source.Type == BlockType.Model && c.Target.Type == BlockType.Solver))
                    issues.Add($"Solver '{solver.Title}' must be connected to the Model block.");
