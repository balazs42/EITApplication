    ...
    internal static class AviVideoWriter
    {
        ...
        internal sealed class AviVideoStream : IDisposable
        {
            ...
            public void WriteFrame(ReadOnlySpan<byte> frameData)
            {
                ...
                uint paddedFrameSize = (uint)((frameData.Length + 1) & ~1);
                _writer.Write(paddedFrameSize);

                long frameStart = _writer.BaseStream.Position - _moviListStart;
                _writer.Write(frameData);

                if ((frameData.Length & 1) != 0)
                    _writer.Write((byte)0);

                _indexEntries.Add(new AviIndexEntry(_chunkFourCc, AviIfKeyFrame, (uint)frameStart, paddedFrameSize));
            }
            ...
            public void Complete()
            {
                ...
                _writer.Write((uint)_indexEntries.Count);

                _writer.BaseStream.Seek(_avihMaxBytesPerSecondPos, SeekOrigin.Begin);
                _writer.Write(maxBytesPerSecond * _framesPerSecond);
                ...
                _writer.BaseStream.Seek(fileEnd, SeekOrigin.Begin);
                _writer.Flush();
                _completed = true;
            }
